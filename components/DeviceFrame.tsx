...
const DeviceFrame: React.FC<DeviceFrameProps> = ({ type, imageSrc, className = '', style = {} }) => {
  ...
  const GlossOverlay = ({ borderRadius }: { borderRadius: string }) => (
    <div className="absolute inset-0 pointer-events-none z-30 opacity-40 mix-blend-soft-light"
         style={{
  ...
  if (type === DeviceType.IPHONE_15_PRO) {
    ...
    return (
      <div className={`relative mx-auto ${className}`} style={{ aspectRatio: '9/19.5', ...style }}>
         ...
         {/* Inset by approx 3% for bezel */}
         <div className="absolute inset-[3%] rounded-[48px] bg-black overflow-hidden z-10">
         ...
         >
            ...
            {/* Main Bezel Black */}
            <path d="M60 12H370C396.51 12 418 33.4903 418 60V872C418 898.51 396.51 920 370 920H60C33.4903 920 12 898.51 12 872V60C12 33.4903 33.4903 12 60 12Z" fill="black"/>
  ...
  if (type === DeviceType.SAMSUNG_S24) {
    return (
      <div className={`relative mx-auto ${className}`} style={{ aspectRatio: '9/19.5', ...style }}>
         {/* Screen Container */}
         <div className="absolute inset-[1.5%] rounded-[4px] bg-black overflow-hidden z-10">
            {renderContent({ borderRadius: '2px' })}
         </div>

         {/* SVG Frame */}
         <svg viewBox="0 0 440 940" className="absolute inset-0 w-full h-full z-20 pointer-events-none">
            <rect x="0" y="0" width="440" height="940" rx="6" fill="#333"/>
            <rect x="4" y="4" width="432" height="932" rx="2" fill="#000"/>
  ...
  if (type === DeviceType.SAMSUNG_S23) {
    return (
      <div className={`relative mx-auto ${className}`} style={{ aspectRatio: '9/19.5', ...style }}>
         <div className="absolute inset-[3%] rounded-[28px] bg-black overflow-hidden z-10">
            {renderContent({ borderRadius: '20px' })}
         </div>

         <svg viewBox="0 0 430 932" className="absolute inset-0 w-full h-full z-20 pointer-events-none">
             <rect x="0" y="0" width="430" height="932" rx="48" fill="#e2e2e2"/>
             <rect x="8" y="8" width="414" height="916" rx="38" fill="black"/>
  ...
  if (type === DeviceType.IPHONE_14_PLUS) {
    return (
       <div className={`relative mx-auto ${className}`} style={{ aspectRatio: '9/19.5', ...style }}>
         <div className="absolute inset-[3.5%] rounded-[40px] bg-black overflow-hidden z-10">
            {renderContent({ borderRadius: '32px' })}
         </div>

         <svg viewBox="0 0 430 932" className="absolute inset-0 w-full h-full z-20 pointer-events-none">
             <rect x="0" y="0" width="430" height="932" rx="55" fill="#f2f2f2" stroke="#d1d1d1" strokeWidth="2"/>
             <rect x="12" y="12" width="406" height="908" rx="46" fill="black"/>
             {/* Notch */}
  ...
  if (type === DeviceType.ANDROID_PIXEL || type === DeviceType.PIXEL_7) {
    ...
    return (
      <div className={`relative mx-auto ${className}`} style={{ aspectRatio: '9/20', ...style }}>
         <div className="absolute inset-[3.5%] rounded-[24px] bg-black overflow-hidden z-10">
            {renderContent({ borderRadius: '20px' })}
         </div>

         <svg viewBox="0 0 412 915" className="absolute inset-0 w-full h-full z-20 pointer-events-none">
             <rect x="0" y="0" width="412" height="915" rx={isPixel7 ? "30" : "40"} fill={frameColor} stroke="#999" strokeWidth="1"/>
             <rect x="10" y="10" width="392" height="895" rx={isPixel7 ? "24" : "34"} fill="black"/>
  ...
  return (
    <div className={`relative mx-auto ${className}`} style={{ aspectRatio: '3/4', ...style }}>
         <div className="absolute inset-[20px] rounded-[16px] bg-black overflow-hidden z-10">
